...
//		hurtingSound = CacheDataLoader.getInstance().getSound("megamanhurt");
	...
	public void jump() {
		if (!getIsJumping()) {
			...
			} else if (getGameWorld().physicalMap.haveCollisionWithRightWall(rectLeftWall) != null && getSpeedX() < 0) {
				setSpeedY(-5.0f);
				flyBackAnim.reset();
				flyForwardAnim.reset();
			}
	...
	public void draw(Graphics2D g2) {
		...
		case ALIVE:
		case NOBEHURT:
			if (getState() == NOBEHURT && (System.nanoTime() / 1000000) % 2 != 1) {
				...
				if (getIsLanding()) {
					...
					if (getDirection() == RIGHT_DIR) {
						...
						if (isShooting) {
							...
							flyShootingForwardAnim.draw((int) (getPosX() - getGameWorld().camera.getPosX()) + 10,
						...
						if (isShooting) {
							...
							flyShootingBackAnim.draw((int) (getPosX() - getGameWorld().camera.getPosX() + 10),
									(int) (getPosY() - getGameWorld().camera.getPosY()), g2);
						} else {
					...
					if (getSpeedX() > 0) {
						...
						if (isShooting) {
							...
							runShootingForwardAnim.draw((int) (getPosX() - getGameWorld().camera.getPosX()),
									(int) getPosY() - (int) getGameWorld().camera.getPosY(), g2);
						} else {
						...
						if (getDirection() == RIGHT_DIR) {
							if (isShooting) {
								...
								idleShootingForwardAnim.draw((int) (getPosX() - getGameWorld().camera.getPosX()),
										(int) getPosY() - (int) getGameWorld().camera.getPosY(), g2);
							} else {
							...
							if (isShooting) {
								...
								idleShootingBackAnim.draw((int) (getPosX() - getGameWorld().camera.getPosX()),
										(int) getPosY() - (int) getGameWorld().camera.getPosY(), g2);
							} else {
